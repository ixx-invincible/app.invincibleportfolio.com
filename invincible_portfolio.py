...
for i in prices.index:
    if i == 0:
        prices.loc[i, 'spy_shares'] = prices['portfolio'][i] * 0.4 / prices['spy'][i]
        prices.loc[i, 'tlt_shares'] = prices['portfolio'][i] * 0.3 / prices['tlt'][i]
        prices.loc[i, 'gld_shares'] = prices['portfolio'][i] * 0.3 / prices['gld'][i]
...
prices.to_excel('out/invincible_portfolio.xlsx')
...
#print(perf['portfolio'].display_monthly_returns())
...
ax1.plot(prices['portfolio'], label='Invincible Portfolio', linewidth=2)
...
plt.savefig('out/invinceible_portfolio.png')
plt.close()


perf['spy'].return_table.to_excel('out/spy_monthly_returns.xlsx')
perf['gld'].return_table.to_excel('out/gld_monthly_returns.xlsx')
perf['tlt'].return_table.to_excel('out/tlt_monthly_returns.xlsx')
perf['portfolio'].return_table.to_excel('out/invincible_portfolio_monthly_returns.xlsx')
